...
export default function TemplateLibrary() {
  ...
  return (
    <div className="p-6 space-y-6" data-testid="template-library">
      ...
      {loading ? (
        <div className="text-sm text-zinc-500 py-8">Loading templates...</div>
      ) : templates.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {templates.map((t) => (
            <div key={t.id} className="border border-border rounded-sm bg-card p-4 space-y-3" data-testid={`template-card-${t.id}`}>
              ...
              <div className="flex gap-1.5 pt-1 border-t border-border">
                <Button variant="outline" size="sm" className="h-7 text-xs flex-1" onClick={() => handleCreateProject(t.id)} data-testid={`use-template-${t.id}`}>
                  <FilePlus2 className="w-3 h-3 mr-1" /> Use
                </Button>
                <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-zinc-500" onClick={() => handleDuplicate(t.id)} title="Duplicate">
                  <Copy className="w-3 h-3" />
                </Button>
                <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-zinc-500" onClick={() => handleToggleLock(t.id)} title={t.locked ? "Unlock" : "Lock"}>
                  {t.locked ? <Unlock className="w-3 h-3" /> : <Lock className="w-3 h-3" />}
                </Button>
                {!t.locked && (
                  <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-zinc-500 hover:text-red-400" onClick={() => handleDelete(t.id, t.name)} title="Delete">
